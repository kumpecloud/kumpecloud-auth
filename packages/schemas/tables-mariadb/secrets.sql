...
create table secrets (
  ...
  encrypted_dek longblob /* @use BufferLike */ not null,
  /** Initialization vector for the secret encryption. */
  iv longblob /* @use BufferLike */ not null,
  /** Authentication tag for the secret encryption. */
  auth_tag longblob /* @use BufferLike */ not null,
  /** The encrypted secret data. e.g. { access_token, refresh_token } */
  ciphertext longblob /* @use BufferLike */ not null,
...
CREATE TRIGGER secrets_set_updated_at BEFORE UPDATE ON secrets FOR EACH ROW SET NEW.updated_at = CURRENT_TIMESTAMP(3);
